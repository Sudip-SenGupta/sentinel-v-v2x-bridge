...
class V2XJNITest {
    ...
    @Test
    fun testPhase1DGroupedUnsignedFixturesProcessSuccessfully() {
        validUnsignedFixtures().forEach { fixture ->
            try {
                val frameType = V2X.detectFrameType(fixture.second)
                assertNotNull("Unsigned fixture ${fixture.first} should expose a frame type", frameType)
                assertTrue("Unsigned fixture ${fixture.first} should expose a non-empty frame type", frameType.isNotEmpty())
            } catch (e: Exception) {
                fail("Unsigned fixture ${fixture.first} failed frame detection: ${e.message}")
            }
        }

        try {
            val result = V2X.processMessage(validUnsignedFixtures().first().second)
            assertNotNull("Unsigned BSM fixture should decode end to end", result)
        } catch (e: Exception) {
            fail("Unsigned BSM fixture failed end-to-end decode: ${e.message}")
        }
    }

    @Test
    fun testPhase1DGroupedSignedFixturesProcessSuccessfully() {
        val generator = V2XSignatureGenerator()
        val builder = COERBinaryMessageBuilder()
        val chain = generator.createCertificateChain(3)
        val fixtures = listOf(
            "signed-bsm-depth3" to generator.generateSignedMessageWithChain(builder.buildBSMPayload(), chain),
            "signed-spat-depth3" to generator.generateSignedMessageWithChain(builder.buildSPaTPayload(), chain),
            "signed-psm-depth3" to generator.generateSignedMessageWithChain(builder.buildPSMPayload(), chain)
        )
        ...
        fixtures.forEach { fixture ->
            try {
                val frameType = V2X.detectFrameType(fixture.second)
                assertNotNull("Signed fixture ${fixture.first} should expose a frame type", frameType)
                assertTrue("Signed fixture ${fixture.first} should expose a non-empty frame type", frameType.isNotEmpty())
            } catch (e: Exception) {
                fail("Signed fixture ${fixture.first} failed frame detection: ${e.message}")
            }
        }

        try {
            val result = V2X.processMessage(fixtures.first().second)
            assertNotNull("Signed BSM fixture should decode end to end", result)
        } catch (e: Exception) {
            fail("Signed BSM fixture failed end-to-end decode: ${e.message}")
        }
    }

    @Test
    fun testPhase1DGroupedMalformedFixturesRejected() {
        val generator = COERMalformationGenerator()

        generator.unsignedParserRejectionCases().forEachIndexed { index, message ->
            assertRejectedByProcessMessage(message, "unsigned-malformed-$index")
        }

        generator.signedParserRejectionCases().forEachIndexed { index, message ->
            assertRejectedByProcessMessage(message, "signed-malformed-$index")
        }
    }

    @Test
    fun testPhase1DValidBatchProcessingStable() {
        val builder = COERBinaryMessageBuilder()
        val unsignedBatch = listOf(
            builder.buildTestBSM(),
            builder.buildTestBSM(),
            builder.buildTestBSM()
        )
        repeat(3) { round ->
            try {
                val decoded = V2X.processBatch(unsignedBatch)
                assertEquals("Round ${round + 1} should decode the full BSM batch", unsignedBatch.size, decoded.size)
            } catch (e: Exception) {
                fail("Round ${round + 1} batch processing failed: ${e.message}")
            }
        }
    }

    private fun assertRejectedByProcessMessage(message: ByteArray, label: String = "malformed message") {
        try {
    ...
    private fun validUnsignedFixtures(): List<Pair<String, ByteArray>> {
        val builder = COERBinaryMessageBuilder()
        return listOf(
            "unsigned-bsm" to builder.buildTestBSM(),
            "unsigned-spat" to builder.buildTestSPaT(),
            "unsigned-psm" to builder.buildTestPSM()
        )
    }
    ...
    @Test
    fun testSignedBSMWithWrongTrustedRootRejectedThroughJNI() {
        val generator = V2XSignatureGenerator()
        val builder = COERBinaryMessageBuilder()
        val chain = generator.createCertificateChain(3)
        val otherChain = generator.createCertificateChain(3)
        ...
        assertTrue("Different trusted root should initialize", V2X.initializeWithRootCA(otherChain.first().first.encoded))
        assertRejectedByProcessMessage(signedBSM, "signed BSM with wrong trusted root")
    }

    @Test
    fun testSignedBSMWithNonCAIntermediateRejectedThroughJNI() {
        val generator = V2XSignatureGenerator()
        val builder = COERBinaryMessageBuilder()
        val chain = generator.createCertificateChain(depth = 3, intermediateIsCa = false)
        val signedBSM = generator.generateSignedMessageWithChain(builder.buildBSMPayload(), chain)
        ...
        assertTrue("Trusted root should initialize", V2X.initializeWithRootCA(chain.first().first.encoded))
        assertRejectedByProcessMessage(signedBSM, "signed BSM with non-CA intermediate")
    }

    @Test
    fun testSignedBSMWithExpiredLeafRejectedThroughJNI() {
        val generator = V2XSignatureGenerator()
        val builder = COERBinaryMessageBuilder()
        val chain = generator.createCertificateChain(
            depth = 3,
        ...
        assertTrue("Trusted root should initialize", V2X.initializeWithRootCA(chain.first().first.encoded))
        assertRejectedByProcessMessage(signedBSM, "signed BSM with expired leaf")
    }

    @Test
    fun testSignedBSMWithNotYetValidLeafRejectedThroughJNI() {
        val generator = V2XSignatureGenerator()
        val builder = COERBinaryMessageBuilder()
        val chain = generator.createCertificateChain(
            depth = 3,
        ...
        assertTrue("Trusted root should initialize", V2X.initializeWithRootCA(chain.first().first.encoded))
        assertRejectedByProcessMessage(signedBSM, "signed BSM with not-yet-valid leaf")
    }

    @Test
    fun testSignedBSMWithLeafPolicyViolationRejectedThroughJNI() {
        val generator = V2XSignatureGenerator()
        val builder = COERBinaryMessageBuilder()
        val chain = generator.createCertificateChain(
            depth = 3,
        ...
        assertTrue("Trusted root should initialize", V2X.initializeWithRootCA(chain.first().first.encoded))
        assertRejectedByProcessMessage(signedBSM, "signed BSM with missing digitalSignature usage")
    }
}
